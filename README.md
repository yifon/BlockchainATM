...
2.每家银行不需要在区块链中预存钱  
3.每台atm因为有取款功能，所以最好是在区块链中预存钱   
4.客人的银行卡假设已经有一个额外的系统转钱到区块链帐户中，所以中区块链中的银行卡余额只代表区块链中的部分，不代表该银行卡在其所属银行的实际余额    
5.假设A行的客人(小王)在B行的ATM(编号B001)上取款500元：  
前提假设：  
  B001:机器中余额100,000  
  小王:余额9,000  
  ATM里的钱是现实货币，与区块链中的数字货币不同，所以凡是涉及到ATM的数额加减，如存款，取款都要记账，BTM(区块链中央银行)会暂时代为收款和扣款，最后ATM平账时会通过第三方系统与ATM所属的银行进行平账。    
发生的确认和交易:  
 a.确保小王在区块链的帐户有500元，若无则应该拒绝交易；  
 b.若有>500元余额，则B001代支出500,余额99,500  
   小王收到500,这个钱是兑现的钱，所以帐户的余额为9,500  
   并在区块链中记账[欠款人：小王；收款人：B001;数额:500]  
### 区块链部分实现
#### 任务 1:
1.1 生成合约地址 -done   
1.2 节点地址 -done   
